      ...
      <ScrollArea className="h-full w-full rounded-md border-1 border-white">
        <div className="p-4 flex flex-col gap-2 ">
            {!messages[0] ? (<p>No messages yet.</p>) : ( messages.map((message)=> (
                <div key={message?._id} className='w-full border-white border-1 rounded-xl flex justify-center p-2 flex-col'>
                    <div className='text-sm'>{message?.sender.username}</div>
                    <div>{message?.content}</div>
                    {user._id === message?.sender._id ? (
                        <div className='mt-4 w-full flex justify-end items-center'>
                            <Button variant='destructive' onClick={()=> deleteMessage(message?._id)}><Trash2/></Button>
                        </div>
                    ) : null}
                </div>
